...
import { createBankMessageInboxForContext } from "./repositories/bank-message-inbox.js";
...
async function main(): Promise<void> {
  ...
  try {
    const complete = await createBankMessageInboxForContext(context, {
      origin: "pasted",
      text: `Compra ficticia ${randomUUID()} de R$ 12,34 em 04/08/2026`,
      consentAccepted: true,
      accountId,
    });
    importBatchIds.push(complete.importBatch.id);
    assert.ok(complete.suggestion);
    ...
    const suggestionRows = await query<{
      ...
      `select "payload", "explanation", "sourceEntityId"
         from "AiSuggestion"
        where "id" = $1 and "organizationId" = $2 and "financialProfileId" = $3`,
      [complete.suggestion.id, organizationId, PERSONAL_PROFILE_ID],
    );
    const row = suggestionRows[0];
    assert.ok(row);
    ...
    if (read.state !== "current" || read.payload.suggestionKind !== "transaction_extraction") {
      throw new Error("Expected a current transaction extraction payload.");
    }
    assert.equal(read.payload.amountMinor, 1234);
    assert.equal(read.payload.accountId, accountId);
    assert.equal(row.sourceEntityId, complete.importBatch.id);
    assert.doesNotMatch(row.explanation, /1234|centavos|conta\s+[0-9a-f-]{36}/i);

    const incomplete = await createBankMessageInboxForContext(context, {
      origin: "pasted",
      text: `Mensagem ficticia sem valor ${randomUUID()}`,
      consentAccepted: true,
      accountId,
    });
    importBatchIds.push(incomplete.importBatch.id);
    assert.equal(incomplete.suggestion, undefined);
    const incompleteSuggestions = await query<{ total: number }>(
      `select count(*)::int as total from "AiSuggestion"
        where "sourceEntityId" = $1 and "organizationId" = $2 and "financialProfileId" = $3`,
      [incomplete.importBatch.id, organizationId, PERSONAL_PROFILE_ID],
    );
    assert.equal(incompleteSuggestions[0]?.total, 0);
  } finally {
