...
async function main(): Promise<void> {
  assert.ok(
    ...
    "DATABASE_URL is required for OFX Unicode scope integration tests.",
  );
  ...
  const invalidDocuments = [
    [
      ...
      `<EXTENSÃO><DTPOSTED>20260729<TRNAMT>-10<FITID>${suffix}-unicode<NAME>Compra`,
    ...
    [
      ...
      `<DTPOSTED>20260729</DTPOSTED><TRNAMT>-10</TRNAMT><FITID>${suffix}-prefix-currency</FITID>`,
      "<NAME>Compra</NAME></STMTTRN></BANKTRANLIST></STMTRS></OFX>",
    ].join(""),
    [
      '<?xml version="1.0"?>',
      "<OFX><STMTRS><CURDEF>BRL</CURDEF><BANKTRANLIST><STMTTRN>",
      `<DTPOSTED>20260729</DTPOSTED><pré:TRNAMT>-10</pré:TRNAMT><FITID>${suffix}-prefix-amount</FITID>`,
      "<NAME>Compra</NAME></STMTTRN></BANKTRANLIST></STMTRS></OFX>",
    ].join(""),
  ];
...
async function assertInvalidRequest(
  ...
  const response = await apiRequest(token, "POST", path, {
    originalFileName: "unicode-container-scope.ofx",
    content,
    accountId,
...
async function createAccount(token: string, suffix: string): Promise<string> {
  const response = await apiRequest(token, "POST", "/api/accounts", {
    name: `Conta escopo Unicode OFX ${suffix}`,
    kind: "checking",
