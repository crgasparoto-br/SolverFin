...
interface BankMessageInboxRow {
  importBatchId: string; organizationId: string; financialProfileId: string; sourceKind: string;
  importStatus: string; originalFileName: string | null; sourceHash: string; receivedAt: Date;
  completedAt: Date | null; importCreatedAt: Date; importUpdatedAt: Date;
  suggestionId: string | null; suggestionKind: string | null; suggestionStatus: string | null;
  targetEntityId: string | null; confidence: string | number | null; explanation: string | null;
  payload: unknown; provider: string | null; model: string | null; reviewedByUserId: string | null;
  reviewedAt: Date | null; suggestionCreatedAt: Date | null; suggestionUpdatedAt: Date | null;
}
...
export class BankMessageInboxRepositoryError extends Error {
  ...
  constructor(code: string, message: string, statusCode = 400) {
    super(message); this.name = "BankMessageInboxRepositoryError"; this.code = code; this.statusCode = statusCode;
  }
}
...
export async function listBankMessageInboxForContext(
  ...
  return rows.map(mapRow).filter((item) => filters.status === undefined || filters.status === "all" || item.status === filters.status);
}
...
export async function createBankMessageInboxForContext(
  ...
  await withTransaction(async (executeQuery) => {
    await executeQuery(
      ...
      [importBatch.id, importBatch.organizationId, importBatch.financialProfileId,
       importBatch.originalFileName ?? null, importBatch.sourceHash, importBatch.receivedAt,
       importBatch.createdAt, importBatch.updatedAt],
    );
      ...
      await executeQuery(
        `insert into "AiSuggestion"
         ...
         values ($1, $2, $3, 'TRANSACTION_EXTRACTION', 'PENDING_REVIEW', $4, null, $5, $6, $7::jsonb,
                 $8, $9, null, null, $10, $11)`,
        [suggestion.id, suggestion.organizationId, suggestion.financialProfileId,
         suggestion.sourceEntityId ?? null, suggestion.confidence, suggestion.explanation,
         JSON.stringify(suggestion.payload), suggestion.provider ?? null, suggestion.model ?? null,
         suggestion.createdAt, suggestion.updatedAt],
      );
    }

    await insertAuditLogEntry(executeQuery, {
      organizationId: context.organizationId, financialProfileId: context.financialProfileId,
      occurredAt: now, actorKind: "user", actorId: context.userId, action: "create",
      entityKind: "import_batch", entityId: importBatch.id,
    ...
    if (suggestion !== undefined) {
      await insertAuditLogEntry(executeQuery, {
        organizationId: context.organizationId, financialProfileId: context.financialProfileId,
        occurredAt: now, actorKind: "system", action: "create", entityKind: "ai_suggestion",
        entityId: suggestion.id, reason: "Sugestao revisavel criada por regra deterministica de inbox.",
  ...
  return {
    id: importBatch.id, origin: payload.origin, status: "pending_review",
    sourceHash: importBatch.sourceHash, maskedText: inboxItem.maskedText,
    receivedAt: importBatch.receivedAt, importBatch,
...
export async function discardBankMessageInboxForContext(
  ...
  await withTransaction(async (executeQuery) => {
    ...
    await insertAuditLogEntry(executeQuery, {
      organizationId: context.organizationId, financialProfileId: context.financialProfileId,
      occurredAt: now, actorKind: "user", actorId: context.userId, action: "soft_delete",
      entityKind: "import_batch", entityId: importBatchId,
      reason: "Mensagem bancaria descartada pelo usuario.", redactedChanges: { status: "changed" },
    });
  });
  return findBankMessageInboxItem(context, importBatchId);
}

async function findBankMessageInboxItem(context: TenantContext, importBatchId: string): Promise<BankMessageInboxItem> {
  const rows = await query<BankMessageInboxRow>(
    `select ${BANK_MESSAGE_SELECT}
     from "ImportBatch" batch
     left join "AiSuggestion" suggestion
  ...
  if (!rows[0]) throw new BankMessageInboxRepositoryError("BANK_MESSAGE_NOT_FOUND", "Mensagem nao encontrada na inbox deste perfil.", 404);
...
function buildImportBatch(
  ...
  return {
    id: item.id, organizationId: context.organizationId, financialProfileId: context.financialProfileId,
    sourceKind: "bank_message", status: "reviewing", originalFileName: `bank-message-${item.origin}`,
    sourceHash: item.sourceHash, receivedAt: item.receivedAt, createdAt: now, updatedAt: now,
    createdByUserId: context.userId, updatedByUserId: context.userId,
  };
}
...
function buildSuggestion(
  ...
  const payload = buildAiSuggestionPayload({
    payload: {
      contractVersion: 1, suggestionKind: "transaction_extraction", payloadVersion: 2,
      origin: { kind: "import", sourceKind: "bank_message", sourceEntityId: importBatch.id },
      target: { entityKind: "transaction" }, confidence,
      ...
      sourceRowNumber: 1, sourceHash: importBatch.sourceHash, occurredOn: parsed.occurredOn,
      kind: parsed.kind, direction: parsed.kind === "income" ? "inflow" : "outflow",
      amountMinor: parsed.amountMinor, currency: "BRL", description: parsed.description,
      ...(input.accountId === undefined ? {} : { accountId: input.accountId }),
      ...(input.categoryId === undefined ? {} : { categoryId: input.categoryId }),
    },
  }) as TransactionExtractionSuggestionPayloadV2;
  return {
    id, organizationId: context.organizationId, financialProfileId: context.financialProfileId,
    kind: "transaction_extraction", status: "pending_review", sourceEntityId: importBatch.id,
    confidence,
    explanation: "Mensagem mascarada analisada. Revise os campos estruturados antes de criar o lancamento.",
    payload, provider: "solverfin-rule-bank-message-inbox", model: "deterministic-v2",
    createdAt: now, updatedAt: now, createdByUserId: context.userId, updatedByUserId: context.userId,
  };
}

function parseBankMessage(maskedText: string): { amountMinor?: number; occurredOn: string; kind: TransactionKind; description: string } {
  const amountMinor = parseAmountMinor(maskedText);
  return {
    ...(amountMinor === undefined ? {} : { amountMinor }),
    occurredOn: parseDate(maskedText) ?? new Date().toISOString().slice(0, 10),
    kind: parseTransactionKind(maskedText), description: normalizeDescription(maskedText),
  };
}
...
function normalizeDescription(text: string): string { return text.replace(/\s+/g, " ").trim().slice(0, 120) || "Mensagem bancaria mascarada"; }

function mapRow(row: BankMessageInboxRow): BankMessageInboxItem {
  const importBatch: ImportBatch = {
    id: row.importBatchId, organizationId: row.organizationId, financialProfileId: row.financialProfileId,
    sourceKind: "bank_message", status: row.importStatus.toLowerCase() as ImportBatch["status"],
    sourceHash: row.sourceHash, receivedAt: row.receivedAt.toISOString(),
    createdAt: row.importCreatedAt.toISOString(), updatedAt: row.importUpdatedAt.toISOString(),
  };
  ...
  return {
    id: row.importBatchId, origin, status: resolveStatus(row.importStatus, suggestion?.status),
    sourceHash: row.sourceHash, maskedText: suggestion?.explanation ?? "Mensagem bancaria mascarada",
    receivedAt: row.receivedAt.toISOString(), importBatch,
    ...(suggestion !== undefined ? { suggestion } : {}),
  };
}

function mapSuggestion(row: BankMessageInboxRow): AiSuggestion | undefined {
  if (row.suggestionId === null || row.suggestionKind === null || row.suggestionStatus === null) return undefined;
  const suggestion: AiSuggestion = {
    id: row.suggestionId, organizationId: row.organizationId, financialProfileId: row.financialProfileId,
...
function resolveStatus(importStatus: string, suggestionStatus: AiSuggestion["status"] | undefined): BankMessageInboxItem["status"] {
...
export function mapBankMessageInboxError(error: unknown): unknown {
  if (error instanceof BankMessageInboxError) return { code: error.code, statusCode: error.statusCode, message: error.message };
  if (error instanceof BankMessageInboxRepositoryError) return { code: error.code, statusCode: error.statusCode, message: error.message };
  return error;
}
