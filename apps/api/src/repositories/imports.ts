...
const IMPORT_BATCH_SELECT_COLUMNS = `"id", "organizationId", "financialProfileId", "defaultAccountId",
  "sourceKind", "status", "originalFileName", "sourceHash", "totalRows", "validRows", "duplicateRows",
...
export async function createCsvImportBatchForContext(
  ...
  await assertActiveAccount(context, payload.accountId, query);
  const preview = await previewCsvImportForContext(context, payload);
  ...
  const duplicate = await findImportBatchBySourceHash(context, preview.batch.sourceHash, query);
