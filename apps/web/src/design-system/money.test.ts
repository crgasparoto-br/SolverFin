...
describe("Money primitive", () => {
  it("reuses the shared formatter with the explicit currency for BRL, USD and EUR", () => {
    ...
    for (const currency of ["BRL", "USD", "EUR"] as const) {
      ...
      assert.ok(markup.includes(expected), `${currency} should preserve the shared formatter output`);
  ...
  it("rejects missing currency before the shared BRL fallback can mask the error", () => {
