...
describe("executable UI primitives", () => {
  it("escapes visible text and attributes while keeping native button semantics", () => {
  ...
  it("models loading, empty, recoverable error, unavailable and permission states explicitly", () => {
    const states = [
    ...
    for (const [index, state] of states.entries()) {
      ...
      assert.ok(state.length > 40, `state ${index} should render useful markup`);
    }

    assert.match(states[0] ?? "", /data-state="loading"[^>]*aria-busy="true"/);
  ...
  it("renders dialog and drawer contracts with a shared modal focus lifecycle", () => {
  ...
  it("keeps navigation tabs keyboard-native instead of claiming a custom tab role", () => {
  ...
  it("allows two distinct route compositions to reuse the same structural primitives", () => {
    ...
    const statement = renderPageContainer({
      childrenHtml:
        ...
        renderDataTable({
          ...
          columns: [
            {
              id: "description",
              header: "Descricao",
              renderCell: (row: { description: string }) => renderText(row.description),
            },
            {
              id: "status",
              header: "Status",
          ...
          rowKey: (_row: { description: string }, index: number) => String(index),
        }),
    });
  ...
  it("renders cards, alerts, feedback and layout composition without financial rules", () => {
    const markup =
      renderCard({
        title: "Resumo",
  ...
  it("rejects empty structural collections instead of emitting ambiguous markup", () => {
    ...
    assert.throws(() => renderTabs({ label: "Abas", items: [] }), /at least one navigation item/);
  });

  it("publishes responsive, focus, overflow and component styles from the shared design system", () => {
    const css = createSolverFinDesignSystemCss();

    assert.match(css, /\.sf-button:focus-visible/);
    assert.match(css, /min-height:\s*var\(--sf-density-interactive-target-min\)/);
    assert.match(css, /\.sf-page-header-title[^}]*overflow-wrap:\s*anywhere/s);
