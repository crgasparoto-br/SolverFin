...
async function accountsCardsPageRendersMasterDetailWithNestedInstruments(): Promise<void> {
  ...
  globalThis.fetch = (async (input: string | URL | Request) => {
    ...
    if (url.endsWith("/api/accounts?status=all")) {
      return jsonResponse({
        accounts: [{
          id: "account-main", name: "Conta pagamento", kind: "checking", status: "active",
          openingBalanceMinor: 0, currency: "USD", institutionKey: "c6",
        }],
      });
    }
    if (url.endsWith("/api/credit-card-accounts?status=all")) {
      return jsonResponse({
        creditCardAccounts: [
          {
            id: "card-c6", name: "Cartão C6", status: "active", closingDay: 20, dueDay: 10,
            creditLimitMinor: 500_000, institutionKey: "c6", brandKey: "mastercard",
            paymentAccountId: "account-main",
            instruments: [
              { id: "instrument-physical", type: "physical", holder: "primary", status: "active", isDefault: true, name: "Físico titular", maskedIdentifier: "**** 1111", creditLimitMinor: 300_000 },
              { id: "instrument-virtual", type: "virtual", holder: "additional", status: "active", isDefault: false, name: "Virtual adicional", maskedIdentifier: "**** 2222", creditLimitMinor: 100_000 },
            ],
          },
          {
            id: "card-blocked", name: "Cartão bloqueado", status: "blocked", closingDay: 5, dueDay: 15,
            creditLimitMinor: 200_000, institutionKey: "nubank", brandKey: "visa", paymentAccountId: "account-main",
            instruments: [{ id: "instrument-archived", type: "virtual", holder: "primary", status: "archived", isDefault: false, name: "Virtual antigo", maskedIdentifier: "**** 9999", creditLimitMinor: 50_000 }],
          },
        ],
      });
    }
  ...
  try {
    ...
    assert.match(html, /data-api-path="\/api\/credit-card-instruments\/instrument-virtual\/archive"/);
...
function adminInstitutionsRouteRequiresSessionButStaysOutOfCommonMenu(): void {
  ...
  const commonShell = renderAuthenticatedShell({ activePathname: "/dashboard", content: "<p>Dashboard</p>", currentLabel: "Dashboard" });
  const masterShell = renderAuthenticatedShell({ activePathname: "/admin/instituicoes", content: "<p>Admin</p>", currentLabel: "Admin - Instituições", showAdminNavigation: true });
...
function accountsCardsPageDoesNotFetchRetiredLinks(): void {
  const source = readFileSync(path.join(repoRoot, "apps", "web", "src", "dev-server", "accounts-cards", "page.ts"), "utf8");
  assert.match(source, /\/api\/credit-card-accounts\?status=all/);
  assert.match(source, /creditCardAccounts/);
  assert.doesNotMatch(source, /\/api\/cards\?status=all/);
  assert.doesNotMatch(source, /\/api\/card-additional-links/);
  assert.doesNotMatch(source, /CardAdditionalLinkRecord/);
}

function accountAndCardInstitutionSelectsUseGlobalCatalog(): void {
  const expectedKeys = ["", ...[...financialInstitutionCatalog]
    .filter((institution) => institution.status === "active")
    .sort((first, second) => first.label.localeCompare(second.label, "pt-BR"))
    .map((institution) => institution.key)];
  assert.deepEqual(institutions.map((institution) => institution.key), expectedKeys);
  for (const catalogInstitution of financialInstitutionCatalog) {
    const webInstitution = institutions.find((institution) => institution.key === catalogInstitution.key);
  ...
  const accountsCardsViewModelSource = readFileSync(path.join(repoRoot, "apps", "web", "src", "dev-server", "accounts-cards", "view-model.ts"), "utf8");
  const accountsCardsPresentationSource = readFileSync(path.join(repoRoot, "apps", "web", "src", "dev-server", "accounts-cards", "presentation.ts"), "utf8");
  const accountsCardsComponentsSource = readFileSync(path.join(repoRoot, "apps", "web", "src", "dev-server", "accounts-cards", "components.ts"), "utf8");
  assert.match(accountsCardsViewModelSource, /import \{ findInstitution \} from "\.\.\/institutions\.js";/);
  assert.match(accountsCardsPresentationSource, /import \{ institutions \} from "\.\.\/institutions\.js";/);
  assert.match(accountsCardsComponentsSource, /import \{ renderInstitutionIcon \} from "\.\.\/institutions\.js";/);
  assert.match(accountsCardsComponentsSource, /<div class="identity-mark">\$\{renderInstitutionIcon/);
  assert.doesNotMatch(accountsCardsComponentsSource, /<div class="identity-mark" aria-hidden="true">\$\{renderInstitutionIcon/);
}
...
function assertLocalInstitutionLogo(src: string): void {
  ...
  if (src.endsWith(".png")) { assert.equal(bytes.subarray(0, 8).toString("hex"), "89504e470d0a1a0a"); return; }
  if (src.endsWith(".svg")) { const svg = bytes.subarray(0, 200).toString("utf8"); assert.match(svg, /<svg\b/i); assert.doesNotMatch(svg, /<script\b/i); return; }
  if (src.endsWith(".webp")) { assert.equal(bytes.subarray(0, 4).toString("ascii"), "RIFF"); assert.equal(bytes.subarray(8, 12).toString("ascii"), "WEBP"); return; }
...
function sidebarMenuUsesPtBrLabels(): void {
  ...
  assert.equal(privateRoutes.has("/contas"), false);
  assert.equal(Array.from(privateRoutes.values()).filter((label) => label === "Contas e Cartões").length, 1);
