...
function renderCardCurrencyOptions(selected?: string): string {
  const placeholder = selected
    ? ""
    : '<option value="" disabled selected>Selecione a moeda</option>';
  return `${placeholder}${renderCurrencyOptions(selected ?? "")}`;
}
  ...
  return renderAccountsCardsDialog({
    ...
    bodyHtml: `
      <form data-api-form data-api-method="PATCH" data-api-path="/api/credit-card-accounts/${escapeHtml(card.id)}" class="edit-grid">
        ...
        <label>Moeda<select name="currency" required>${renderCardCurrencyOptions(card.currency)}</select></label>
        ...
        <label>Limite total (moeda do cartão)<input name="creditLimitMinor" data-money value="${formatMoneyInput(card.creditLimitMinor ?? 0)}" inputmode="decimal" /></label>
...
function renderCardInlineInstrumentForm(instrument: CardInstrumentRecord): string {
  ...
  return `
    <form data-api-form data-api-method="PATCH" data-api-path="/api/credit-card-instruments/${escapeHtml(instrument.id)}" class="edit-grid instrument-edit-form">
      ...
      <label>Limite do instrumento (moeda do cartão)<input name="creditLimitMinor" data-money value="${instrument.creditLimitMinor !== undefined ? formatMoneyInput(instrument.creditLimitMinor) : ""}" inputmode="decimal" /></label>
  ...
  return renderAccountsCardsDialog({
    ...
    bodyHtml: `
      <form data-api-form data-api-path="/api/credit-card-accounts/${escapeHtml(card.id)}/instruments" class="edit-grid">
        ...
        <label>Limite do instrumento (moeda do cartão)<input name="creditLimitMinor" data-money inputmode="decimal" placeholder="0,00" /></label>
  ...
  return renderAccountsCardsDialog({
    ...
    bodyHtml: `
      <form data-api-form data-api-path="/api/credit-card-accounts" data-payload-kind="credit-card-account" class="edit-grid">
        ...
        <label>Moeda<select name="currency" required>${renderCardCurrencyOptions()}</select></label>
        ...
        <label>Limite total (moeda do cartão)<input name="creditLimitMinor" data-money inputmode="decimal" placeholder="0,00" /></label>
        ...
        <label>Limite do instrumento (moeda do cartão)<input name="instrumentCreditLimitMinor" data-money inputmode="decimal" placeholder="0,00" /></label>
