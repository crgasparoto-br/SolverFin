...
describe("budgets page issue 613", () => {
  it("renders A1 with explicit currencies and backend realized values without future projections", async () => {
    globalThis.fetch = async (input: string | URL | Request): Promise<Response> => {
      ...
      if (url.pathname === "/api/categories") {
        return json({ categories: [{ id: "food", name: "Alimentação", kind: "expense", status: "active" }] });
      }
  ...
  it("does not turn a mismatched usage currency into a realized zero", async () => {
    globalThis.fetch = async (input: string | URL | Request): Promise<Response> => {
      const url = new URL(String(input), "http://solverfin.test");
      if (url.pathname === "/api/budgets") return json({ budgets: [budget("budget-brl", "BRL", 100_000)] });
      if (url.pathname === "/api/categories") return json({ categories: [{ id: "food", name: "Alimentação", kind: "expense", status: "active" }] });
      if (url.pathname === "/api/budgets/budget-brl/usage") {
  ...
  it("keeps creation and editing contextual with explicit currency", async () => {
    globalThis.fetch = async (input: string | URL | Request): Promise<Response> => {
      ...
      if (url.pathname === "/api/budgets") return json({ budgets: [budget("budget-brl", "BRL", 100_000)] });
      if (url.pathname === "/api/categories") return json({ categories: [{ id: "food", name: "Alimentação", kind: "expense", status: "active" }] });
      if (url.pathname === "/api/budgets/budget-brl/usage") return json({ usage: usage("budget-brl", "BRL", 100_000, 40_000, 40) });
      return json({});
    };
