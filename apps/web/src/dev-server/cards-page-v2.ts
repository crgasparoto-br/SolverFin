...
function renderPurchaseRow(
  ...
  return `<article class="cards-purchase-row" role="row" data-purchase-item data-reconciliation="${purchase.status === "reconciled" ? "reconciled" : "unreconciled"}">
    ...
    <div role="cell" data-label="Compra" class="cards-purchase-description"><strong>${escapeHtml(purchase.description)}${recurrence ? renderRecurrenceIndicator() : ""}</strong><span>${escapeHtml(category)}</span></div>
