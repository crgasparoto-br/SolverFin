      ...
      <section class="cards-layout">
        ...
        <section class="panel invoice-panel">
          <div class="invoice-toolbar">
            <div>
              <p class="eyebrow">Compras e parcelas</p>
  ...
  return `
    <script>
      ...
      function syncPurchaseFieldVisibility() {
        if (!purchaseForm || !purchaseRepeatMode) return;
...
function css(): string {
  return `
    ...
    .cards-heading { align-items: center; display: flex; gap: 12px; justify-content: space-between; } .cards-heading > div { display: grid; gap: 4px; max-width: 760px; }
    .card-filter { background: var(--surface); }
    .filter-form { align-items: end; display: grid; gap: 10px; grid-template-columns: minmax(12rem, 1.2fr) minmax(11rem, 1fr); }
    .account-field { align-items: end; display: grid; gap: 6px; grid-template-columns: 1fr auto; position: relative; } .account-field > label:first-child { grid-column: 1 / -1; }
    .ghost-link { align-items: center; background: var(--primary-soft); border: 1px solid #d4e6ec; border-radius: var(--radius); color: var(--primary); display: inline-flex; height: 36px; justify-content: center; width: 36px; } .ghost-link:hover { background: #d4e6ec; }
    .account-select { position: relative; } .account-select-trigger { align-items: center; background: var(--surface); border: 1px solid var(--line); color: var(--text); display: flex; gap: 8px; justify-content: flex-start; min-height: 36px; padding: 0 10px; text-align: left; width: 100%; } .account-select-trigger:hover { background: var(--primary-soft); } .account-select-icon { align-items: center; display: inline-flex; flex-shrink: 0; height: 20px; width: 20px; } .account-select-icon .brand-icon, .account-select-icon .brand-icon-wrap { display: block; height: 20px; width: 20px; } .account-select-icon .brand-logo-img { background: #fff; border-radius: 50%; object-fit: contain; padding: 2px; } .account-select-text { flex: 1; font-size: 0.875rem; font-weight: 600; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; } .account-select-chevron { color: var(--muted); flex-shrink: 0; } .account-select-menu { background: var(--surface); border: 1px solid var(--line); border-radius: var(--radius); box-shadow: 0 12px 32px rgba(15,23,42,.14); left: 0; list-style: none; margin: 4px 0 0; max-height: 260px; overflow-y: auto; padding: 4px; position: absolute; right: 0; top: 100%; z-index: 20; } .account-select-menu li { align-items: center; border-radius: var(--radius); cursor: pointer; display: flex; font-size: 0.875rem; font-weight: 600; gap: 8px; padding: 7px 8px; } .account-select-menu li:hover, .account-select-menu li[aria-selected=true] { background: var(--primary-soft); }
    .month-field { display: grid; gap: 6px; } .month-nav { align-items: center; background: var(--bg); border: 1px solid var(--line); border-radius: var(--radius); display: grid; gap: 4px; grid-template-columns: auto minmax(0,1fr) auto; padding: 3px; } .month-current { font-size: 0.875rem; font-weight: 600; text-align: center; } .icon-btn { background: var(--surface); border: 1px solid var(--line); border-radius: var(--radius); color: var(--primary); min-height: 30px; min-width: 30px; padding: 0; } .icon-btn:hover { background: var(--primary-soft); }
    .cards-layout { align-items: start; display: grid; gap: 12px; grid-template-columns: 270px minmax(0,1fr); } .invoice-summary { display: grid; gap: 12px; position: sticky; top: 68px; }
    .summary-block { border-top: 1px solid var(--line); display: grid; gap: 8px; padding-top: 10px; } .summary-block:first-child { border-top: 0; padding-top: 0; } .summary-block h2 { font-size: 0.8125rem; }
    .summary-list { display: grid; gap: 4px; } .summary-row { align-items: center; display: flex; font-size: 0.8125rem; gap: 8px; justify-content: space-between; } .summary-row dt { color: var(--muted); font-weight: 600; } .summary-row dd { font-weight: 700; text-align: right; } .summary-row-strong dd { font-size: 0.9375rem; }
    .invoice-actions { display: grid; gap: 6px; }
    .credit { color: var(--success) !important; } .debit { color: var(--danger) !important; }
    .invoice-panel { display: grid; gap: 12px; padding: 0; } .invoice-toolbar { align-items: center; border-bottom: 1px solid var(--line); display: flex; flex-wrap: wrap; gap: 10px; justify-content: space-between; padding: 12px 14px; }
    .filter-controls { align-items: center; display: flex; flex-wrap: wrap; gap: 6px; } .filter-controls input[type=search] { min-width: 200px; width: auto; }
    .toggle-chip { background: var(--bg); border: 1px solid var(--line); border-radius: 999px; color: var(--muted); font-size: 0.75rem; font-weight: 600; min-height: 28px; padding: 0 10px; } .toggle-chip[aria-pressed=true] { background: var(--primary-soft); border-color: #a5cbd6; color: var(--primary); }
    .purchase-list { display: grid; gap: 0; padding: 0 14px 14px; } .purchase-group { border-top: 1px solid var(--line); } .purchase-group:first-child { border-top: 0; } .purchase-group-summary { align-items: center; cursor: pointer; display: flex; gap: 8px; list-style: none; padding: 10px 0; } .purchase-group-summary::-webkit-details-marker { display: none; } .purchase-group-summary::before { color: var(--muted); content: "▸"; transition: transform .15s ease; } .purchase-group[open] > .purchase-group-summary::before { transform: rotate(90deg); } .purchase-group-name { font-size: 0.875rem; font-weight: 700; } .purchase-group-summary .muted { flex: 1; }
    .purchase-group-rows { display: grid; gap: 0; padding-bottom: 4px; } .purchase-group-rows .purchase-row:first-child { border-top: 1px solid var(--line); } .purchase-row { align-items: center; border-top: 1px solid var(--line); display: grid; gap: 10px; grid-template-columns: 6rem minmax(0,1fr) auto 8rem 3rem; padding: 8px 0; } .purchase-row:first-child { border-top: 0; } .purchase-row time { color: var(--muted); font-size: 0.8125rem; }
    .description { display: grid; gap: 2px; min-width: 0; } .description span { color: var(--muted); font-size: 0.8125rem; }
    .chip { align-items: center; background: var(--primary-soft); border: 1px solid #d4e6ec; border-radius: 999px; color: var(--primary); display: inline-flex; font-size: 0.6875rem; font-weight: 700; gap: 4px; padding: 2px 7px; white-space: nowrap; } .chip-ok { background: var(--success-bg); border-color: #bbf7d0; color: var(--success); } .chip-posted { background: #e0f2fe; border-color: #bae6fd; color: #0369a1; }
    .actions { position: relative; } .actions summary { align-items: center; background: var(--primary-soft); border: 1px solid #d4e6ec; border-radius: 999px; color: var(--primary); cursor: pointer; display: inline-flex; height: 28px; justify-content: center; list-style: none; width: 28px; } .actions summary::-webkit-details-marker { display: none; } .actions summary:hover { background: #d4e6ec; } .actions-menu { background: var(--surface); border: 1px solid var(--line); border-radius: var(--radius); box-shadow: 0 12px 32px rgba(15,23,42,.14); display: grid; gap: 2px; padding: 4px; position: absolute; right: 0; top: 34px; width: max-content; z-index: 50; } .actions-item { align-items: center; background: transparent; border: 0; border-radius: var(--radius); color: var(--text); display: flex; font-size: 0.8125rem; font-weight: 600; gap: 8px; justify-content: flex-start; min-height: 30px; padding: 0 8px; text-align: left; white-space: nowrap; } .actions-item:hover { background: var(--primary-soft); }
    .empty-state { background: var(--bg); border: 1px dashed var(--line); border-radius: var(--radius-lg); display: grid; gap: 4px; margin: 14px; padding: 14px; }
    dialog { border: 0; border-radius: var(--radius-lg); box-shadow: 0 24px 80px rgba(15,23,42,.24); max-width: min(640px, calc(100vw - 32px)); padding: 0; width: 100%; } dialog::backdrop { background: rgba(6,25,35,.48); } .modal-panel { display: grid; gap: 14px; padding: 18px; } .close-form { display: flex; justify-content: flex-end; } .modal-panel form:not(.close-form) { display: grid; gap: 10px; grid-template-columns: repeat(2, minmax(0,1fr)); } .full { grid-column: 1 / -1; }
    .error-page { min-height: 100vh; place-content: center; }
    @media (max-width: 1080px) { .cards-layout { grid-template-columns: 1fr; } .invoice-summary { position: static; } }
    @media (max-width: 760px) { main { padding: 14px 14px 24px; } .filter-form, .modal-panel form:not(.close-form) { grid-template-columns: 1fr; } .cards-heading { align-items: stretch; display: grid; } .purchase-group-summary { align-items: flex-start; display: grid; } .purchase-row { align-items: start; grid-template-columns: 1fr; } }
    ${recurrencesSectionStyles()}
  `;
}
