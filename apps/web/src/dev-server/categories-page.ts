...
function pageCss(): string {
  return `
    ${sharedShellStyles()}
    [hidden] { display: none !important; } body.modal-open { overflow: hidden; } h1 { font-size: clamp(1.65rem, 4vw, 2.15rem); line-height: 1.12; }
    input, select { background: var(--surface); border: 1px solid var(--line); border-radius: 8px; color: var(--text); font: inherit; min-height: 44px; padding: 0 12px; width: 100%; } label { color: var(--text); display: grid; font-weight: 700; gap: 8px; }
