...
function pageCss(): string {
  return `
    ...
    * { box-sizing: border-box; } [hidden] { display: none !important; } body { margin: 0; min-height: 100vh; background: var(--bg); color: var(--text); font-family: Inter, ui-sans-serif, system-ui, -apple-system, BlinkMacSystemFont,"Segoe UI",sans-serif; } body.modal-open { overflow: hidden; } h1, h2, h3, p { margin: 0; } h1 { font-size: clamp(1.65rem, 4vw, 2.15rem); line-height: 1.12; } h2 { font-size: 1rem; line-height: 1.3; } h3 { font-size: .95rem; line-height: 1.3; } a { color: inherit; }
    ...
    .category-directory { padding: 0; overflow: hidden; } .category-toolbar { align-items: start; border-bottom: 1px solid var(--line); display: flex; gap: 16px; justify-content: space-between; padding: 18px; } .category-toolbar > div:first-child { display: grid; gap: 4px; min-width: 0; } .category-toolbar-actions { display: grid; gap: 10px; justify-items: end; } .category-collapse-actions { display: flex; flex-wrap: wrap; gap: 8px; justify-content: flex-end; } .compact-button { min-height: 34px; padding: 0 12px; } .filter-chips { display: flex; flex-wrap: wrap; gap: 8px; justify-content: flex-end; } .filter-chip { background: var(--primary-soft); border: 1px solid #d4e6ec; color: var(--primary); min-height: 34px; padding: 0 12px; } .filter-chip[aria-pressed="true"] { background: var(--primary); border-color: var(--primary); color: white; }
