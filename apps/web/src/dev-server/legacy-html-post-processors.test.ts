...
test("legacy HTML post-processors have a canonical, reducing inventory", () => {
  assert.equal(
    LEGACY_HTML_POST_PROCESSOR_INVENTORY.length,
    LEGACY_HTML_POST_PROCESSOR_BUDGET,
  );
  ...
  const expectedOwnerByRoute: Record<
    LegacyHtmlPostProcessorRoute,
    LegacyHtmlPostProcessorOwner
  > = {
...
test("legacy HTML pipeline preserves the registered order including async adapters", async () => {
  const result = await applyLegacyHtmlPostProcessorPipeline(
    "/categorias",
    "rendered",
    [
      {
