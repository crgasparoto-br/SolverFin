...
export function renderAnalysisSummaryGrid(items: readonly AnalysisSummaryItem[]): string {
    ...
    childrenHtml: items
      .map((item) => {
        const secondary = item.secondaryHtml
          ...
          : "";
        return `<article class="report-summary-metric" data-tone="${item.tone ?? "neutral"}"><span>${renderText(item.label)}</span><strong>${item.primaryHtml}</strong>${secondary}</article>`;
      })
      .join(""),
  });
}
...
export function renderResultTrend(points: readonly TrendPoint[]): string {
  ...
  return `<ol class="report-trend" aria-label="Resultado por período">${points
    .map((point) => {
      const scale = point.amountMinor === 0 ? 0 : Math.max(4, Math.round((Math.abs(point.amountMinor) / maxAbsolute) * 100));
      const sign = point.amountMinor < 0 ? "negative" : point.amountMinor > 0 ? "positive" : "neutral";
  ...
  const negativeAlert =
    input.negativePeriodCount > 0
      ? renderAlert({
          ...
          description: "Revise a evolução e o detalhe por categoria antes de decidir qualquer ajuste.",
        })
      : renderAlert({
          tone: "positive",
          title: "Nenhum período com resultado negativo",
          description: "Use a matriz detalhada para entender quais categorias sustentam o resultado.",
        });
