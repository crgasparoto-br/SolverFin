...
function css(): string {
  return `
    ...
    .reports-heading { align-items: center; display: flex; gap: 12px; justify-content: space-between; } .reports-heading > div { display: grid; gap: 4px; max-width: 760px; }
    .readonly-pill { background: var(--primary-soft); border: 1px solid #d4e6ec; border-radius: 999px; color: var(--primary); font-size: 0.75rem; font-weight: 700; padding: 3px 10px; white-space: nowrap; }
    .report-filter-panel { padding: 12px 14px; }
    .report-filters { align-items: end; display: grid; gap: 10px; grid-template-columns: minmax(9rem,.8fr) minmax(9rem,.8fr) minmax(11rem,1fr) minmax(11rem,1fr) auto; }
    ...
    .metric-card strong { color: var(--primary); font-size: 1.25rem; font-weight: 700; line-height: 1.1; }
    .metric-card p { color: var(--muted); font-size: 0.8125rem; }
    .metric-primary { background: var(--primary); border-color: var(--primary); color: white; } .metric-primary span, .metric-primary strong, .metric-primary p { color: white; }
    .metric-warning { background: var(--warning-bg); border-color: #fde68a; } .metric-warning strong, .metric-warning p { color: var(--warning); }
    .report-grid { align-items: start; display: grid; gap: 12px; grid-template-columns: repeat(3, minmax(0,1fr)); } .report-results { min-width: 0; }
    .section-heading { align-items: center; display: flex; gap: 10px; justify-content: space-between; } .section-heading > div { display: grid; gap: 3px; } .section-heading span { background: var(--primary-soft); border-radius: 999px; color: var(--primary); font-size: 0.6875rem; font-weight: 700; padding: 2px 7px; white-space: nowrap; }
    .aggregate-list { display: grid; gap: 0; } .aggregate-row { align-items: center; border-top: 1px solid var(--line); display: flex; gap: 10px; justify-content: space-between; padding: 8px 0; } .aggregate-row:first-child { border-top: 0; } .aggregate-row div { display: grid; gap: 2px; min-width: 0; } .aggregate-row span { color: var(--muted); font-size: 0.8125rem; } .aggregate-row > strong { font-size: 0.875rem; white-space: nowrap; }
    .installment-table { display: grid; overflow-x: auto; } .installment-table-head, .installment-table-row { align-items: center; border-bottom: 1px solid var(--line); display: grid; gap: 10px; grid-template-columns: 6rem 4rem minmax(11rem,1.2fr) minmax(8rem,1fr) minmax(8rem,1fr) minmax(9rem,1fr) 7rem; min-width: 860px; padding: 8px 0; } .installment-table-head { color: var(--muted); font-size: 0.6875rem; font-weight: 700; text-transform: uppercase; } .installment-table-row:last-child { border-bottom: 0; } .installment-table-row time, .installment-table-row span { color: var(--muted); font-size: 0.8125rem; } .installment-table-row strong:last-child { text-align: right; white-space: nowrap; }
    @media (max-width: 1180px) { .summary-grid { grid-template-columns: repeat(3, minmax(0,1fr)); } .report-grid { grid-template-columns: 1fr 1fr; } .report-filters { grid-template-columns: repeat(2, minmax(0,1fr)); } .report-filters button { grid-column: 1 / -1; } }
    @media (max-width: 760px) { main { padding: 14px 14px 24px; } .reports-heading, .section-heading { align-items: stretch; display: grid; } .summary-grid, .report-grid, .report-filters { grid-template-columns: 1fr; } .readonly-pill { justify-self: start; } }
  `;
}
