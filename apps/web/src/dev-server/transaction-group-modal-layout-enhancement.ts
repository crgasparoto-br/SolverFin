...
export function enhanceTransactionGroupModalLayout(html: string): string {
  ...
  const styles = `
    <style ${STYLE_MARKER}>
      dialog[data-group-modal]{box-sizing:border-box;height:fit-content;max-height:min(92dvh,860px);max-width:min(1220px,calc(100vw - 24px));overflow:hidden;width:min(1220px,calc(100vw - 24px))}
      dialog[data-group-modal] .group-modal-panel{box-sizing:border-box;height:fit-content;max-height:min(92dvh,860px);min-width:0;padding:20px 22px 18px;width:100%}
      ...
      dialog[data-group-modal] .group-modal-panel form[data-group-form]{flex:0 1 auto;grid-auto-flow:row dense;grid-template-columns:repeat(12,minmax(0,1fr));max-height:calc(92dvh - 92px);min-width:0;overflow-x:hidden;overflow-y:auto;padding:4px 2px 0;scrollbar-gutter:stable}
      ...
      dialog[data-group-modal] .group-members{box-sizing:border-box;max-height:clamp(220px,36vh,420px);min-width:0;overflow-x:hidden;overflow-y:auto;overscroll-behavior:contain;scrollbar-gutter:stable;width:100%}
      ...
      @media(max-width:760px){
        dialog[data-group-modal]{height:fit-content;max-height:calc(100dvh - 12px);max-width:calc(100vw - 12px);width:calc(100vw - 12px)}
        dialog[data-group-modal] .group-modal-panel{height:fit-content;max-height:calc(100dvh - 12px);padding:16px 14px 14px;width:100%}
