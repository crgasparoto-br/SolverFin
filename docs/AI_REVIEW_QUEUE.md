...
`AiSuggestion.payload` armazena somente dados normalizados e versionados. Para importação CSV, novas sugestões `transaction_extraction` usam `TransactionExtractionPayloadV2`, com linha de origem, hash, data, tipo revisado, direção original `inflow|outflow`, valor, moeda, descrição, conta de referência, outra conta para transferências, categoria e ID externo opcionais. V1 permanece legível; sugestões pendentes derivam a direção pelo tipo anterior e migram somente quando necessário.
