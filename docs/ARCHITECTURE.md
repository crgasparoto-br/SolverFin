...
- `apps/web` e um servidor SSR em Node `http` puro que autentica contra a API real (token guardado em cookie HttpOnly) e renderiza dashboard, contas, categorias, lancamentos, recorrencias, contas a pagar/receber, cartoes e orcamentos com dados reais; demais rotas do menu ainda sao placeholder ou fluxos parciais.
- Recorrencias possuem rota web inicial em `/recorrencias` para listar, criar, editar, pausar, retomar, cancelar e gerar parcelas. Parcelas geradas aparecem como retorno imediato da acao de geracao, mas ainda nao ha rota dedicada para reler parcelas historicas por recorrencia.
