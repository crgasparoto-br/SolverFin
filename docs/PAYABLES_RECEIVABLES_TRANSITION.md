...
Para `pending` do tipo `payable`, uma `Invoice` equivalente mantem o registro legado como
referencia; a `Invoice` permanece a obrigacao canonica de caixa do cartao.
...
2. leia tambem `Invoice` e `Card` do mesmo tenant/perfil e gere um plano usando
   `buildPayableReceivableTransitionPlan`;
...
- Para `Invoice`, a equivalencia legada exige payable no mesmo tenant/perfil, mesmo valor, moeda e
  vencimento e, quando o legado informa `accountId`, a mesma conta de pagamento do cartao.
  Receivables e contas divergentes permanecem distintos.
