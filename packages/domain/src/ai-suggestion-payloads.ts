...
export function readAiSuggestionPayload(
  ...
  try {
    if (isRecord(value) && "contractVersion" in value) {
      return { state: "current", payload: validateAiSuggestionPayload(value, expectedKind) };
    }
...
export function mutateAiSuggestionPayload<TPayload extends AiSuggestionPayload>(input: {
  ...
  const current = validateAiSuggestionPayload(input.payload, input.payload.suggestionKind) as TPayload;
  ...
  const withAudit = { ...next, audit: input.audit, fingerprint: "pending" } as TPayload;
