...
test("generated classifier package is intact and pinned to the orchestrator source", () => {
  const verify = spawnSync(
    process.execPath,
    [fileURLToPath(new URL("../.delivery-v2/verify.mjs", import.meta.url))],
    { encoding: "utf8" },
  );
  assert.equal(verify.status, 0, verify.stderr);
  const lock = JSON.parse(
    readFileSync(new URL("../.delivery-v2/lock.json", import.meta.url), "utf8"),
  );
...
test("workflow and classifier changes are CRITICAL", () => {
  ...
  assert.equal(
    classifyDeliveryV2Ci({ changedPaths: ["scripts/delivery-v2-ci-classifier.mjs"] }).riskProfile,
    "critical",
  );
});
