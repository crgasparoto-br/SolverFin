...
const evidence = { commit: process.env.GITHUB_SHA ?? "local", browser: browser.version };

try {
  await setViewport(browser.cdp, 1280, 900);
  await navigate(browser.cdp, "about:blank");
  await evaluate(browser.cdp, `(() => { document.open(); document.write(${JSON.stringify(html)}); document.close(); })()`);
  ...
  evidence.drawer = await exerciseModal("sample-drawer-trigger", "sample-drawer", "drawer", "sample-dialog-trigger");
  ...
  await writeFile(join(outputDir, "issue-601-ui-primitives.json"), `${JSON.stringify(evidence, null, 2)}\n`);
...
async function exerciseModal(triggerId, modalId, kind, previousTriggerId) {
  await evaluate(browser.cdp, previousTriggerId
    ? `document.getElementById(${JSON.stringify(previousTriggerId)}).focus()`
    : `document.activeElement instanceof HTMLElement && document.activeElement.blur()`);
  await pressKey("Tab");
  const focused = await evaluate(browser.cdp, `(() => {
  ...
  const opened = await evaluate(browser.cdp, `document.getElementById(${JSON.stringify(modalId)}).open === true`);
...
async function pressKey(key) {
  ...
  const common = { key, code: key, windowsVirtualKeyCode: keyCode, nativeVirtualKeyCode: keyCode };
  await browser.cdp.send("Input.dispatchKeyEvent", { type: "rawKeyDown", ...common });
  if (key === "Enter") await browser.cdp.send("Input.dispatchKeyEvent", { type: "char", ...common, text: "\r", unmodifiedText: "\r" });
  await browser.cdp.send("Input.dispatchKeyEvent", { type: "keyUp", ...common });
  await sleep(80);
}

function fixtureHtml() {
  const long = "LongUnbrokenIssue601TitleThatMustWrapWithoutHorizontalDocumentOverflow1234567890".repeat(6);
  const columns = Array.from({ length: 8 }, (_, index) => ({
    id: `c${index}`,
    header: `Column ${index + 1}`,
    renderCell: (row) => `<span style="display:inline-block;min-width:10rem">${renderText(row[`c${index}`])}</span>`,
  }));
  const row = Object.fromEntries(columns.map((column, index) => [column.id, `Cell ${index + 1} long content`]));
  const table = renderDataTable({ caption: "Responsive issue 601 table", columns, rows: [row], rowKey: () => "row" });
  const trigger = (id, label) => renderDialogTrigger({ dialogId: id, label }).replace("<button ", `<button id="${id}-trigger" `);
  const dialog = renderDialog({ id: "sample-dialog", title: "Dialog", bodyHtml: renderText("Dialog body") });
  const drawer = renderDrawer({ id: "sample-drawer", title: "Drawer", bodyHtml: renderText("Drawer body") });
  const content = renderPageContainer({ childrenHtml:
    renderPageHeader({ title: long, actionsHtml: `${trigger("sample-dialog", "Open dialog")}${trigger("sample-drawer", "Open drawer")}` }) +
    renderDetailLayout({ masterHtml: renderText("Master"), detailHtml: table }) + dialog + drawer,
  });
