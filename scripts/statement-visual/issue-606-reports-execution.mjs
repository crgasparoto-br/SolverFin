...
async function validateEmpty(cdp) {
  ...
  report.evidence.push({ width: 1024, height: 700, screenshot: screenshotName, measurements });
}
...
async function validateFilterError(cdp) {
  ...
  assert.ok(measurements.invalidControls >= 3, "Reports invalid filters are not identified accessibly.");
  assert.equal(measurements.noHorizontalOverflow, true, "Reports filter-error state has page overflow.");
  report.evidence.push({ width: 1024, height: 700, screenshot: screenshotName, measurements });
}
