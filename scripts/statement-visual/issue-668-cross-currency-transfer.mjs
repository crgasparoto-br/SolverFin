...
try {
  ...
  const route = `/lancamentos?accountId=${encodeURIComponent(fixture.sourceAccountId)}&month=2026-09`;
  ...
  check(desktop.effectiveRate === "0,185763", "Derived effective rate is not deterministic", desktop);
  check(desktop.effectiveRateDirection === "USD por BRL", "Effective-rate direction is ambiguous", desktop);
  check(desktop.repeatMode === "single", "Cross-currency transfer did not force single occurrence", desktop);
  check(desktop.installmentDisabled, "Installment option remains enabled for cross-currency transfer", desktop);
  check(desktop.repeatHintVisible, "Cross-currency recurrence hint is not visible", desktop);
  check(!desktop.globalOverflow, "Cross-currency transfer modal overflows desktop viewport", desktop);
  ...
  check(currencyChange.destinationCurrency === "EUR", "Destination currency did not switch to EUR", currencyChange);
  check(currencyChange.destinationAmount === "", "Stale USD destination amount survived EUR destination change", currencyChange);
  check(currencyChange.effectiveRate === "Informe os dois valores", "Rate was not invalidated after destination currency change", currencyChange);
  ...
  check(mobile.destinationAmountVisible, "Destination native amount is not visible on mobile", mobile);
  check(mobile.effectiveRateVisible, "Effective rate is not visible on mobile", mobile);
  check(mobile.effectiveRateDirection === "USD por BRL", "Rate direction changed on mobile", mobile);
  ...
  check(saved.requestBody?.amountMinor === 53_832, "Source native amount changed in form payload", saved);
  check(saved.requestBody?.destinationAmountMinor === 10_000, "Destination native amount changed in form payload", saved);
  check(saved.persisted?.currency === "BRL", "Persisted source currency is not BRL", saved);
  check(saved.persisted?.destinationCurrency === "USD", "Persisted destination currency is not USD", saved);
  check(saved.persisted?.amountMinor === 53_832, "Persisted source native amount is incorrect", saved);
  check(saved.persisted?.destinationAmountMinor === 10_000, "Persisted destination native amount is incorrect", saved);
} finally {
