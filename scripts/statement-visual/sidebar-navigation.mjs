...
async function validateDesktopSidebar(cdp) {
  ...
  check(measurements.sidebar.fillsViewport, "Sidebar does not fill the viewport height", measurements);
  ...
  check(measurements.navigation.isScrollable, "Navigation does not have vertical overflow", measurements);
  check(measurements.navigation.scrollTop > 0, "Tab traversal did not scroll navigation", measurements);
  check(measurements.navigation.lastLinkInside, "Last authorized link is outside navigation", measurements);
  check(measurements.navigation.focusedTarget, "Tab traversal did not reach the last link", measurements);
  check(measurements.pageScrollY === 0, "Main page scrolled instead of the navigation region", measurements);
  check(measurements.admin.institutions === 1, "Admin institutions link is missing or duplicated", measurements);
  check(
    measurements.admin.financialIndexes === 1,
    "Financial indexes link is missing or duplicated",
    measurements,
  );
  check(measurements.admin.groupLabels === 1, "Admin group label is missing or duplicated", measurements);
  check(measurements.duplicateRouteIds.length === 0, "Navigation route ids are duplicated", measurements);
  check(measurements.ariaControlsComplete, "aria-controls does not cover every secondary route", measurements);
  check(measurements.activeRoute === "dashboard", "Dashboard active state was not preserved", measurements);
  check(
...
async function validateMobileNavigation(cdp) {
  ...
  check(before.toggleFocused, "Tab traversal did not reach the More/Less button", { before, traversal });
  ...
  check(opened.adminInstitutionsVisible, "Admin institutions is not visible after expansion", opened);
  check(opened.adminFinancialIndexesVisible, "Financial indexes is not visible after expansion", opened);
  check(opened.duplicateRouteIds.length === 0, "Opening More/Less duplicated navigation routes", opened);
  check(opened.ariaControlsComplete, "Mobile aria-controls is incomplete after expansion", opened);
  check(opened.internalVerticalScroll === false, "Mobile navigation gained internal vertical scroll", opened);
  ...
  check(closed.adminInstitutionsVisible === false, "Admin institutions remained visible after collapse", closed);
  check(
    closed.adminFinancialIndexesVisible === false,
    "Financial indexes remained visible after collapse",
    closed,
  );
  check(closed.duplicateRouteIds.length === 0, "Closing More/Less duplicated navigation routes", closed);
...
async function resetPageFocus(cdp) {
  await evaluate(cdp, `(() => { document.activeElement?.blur(); return document.activeElement?.tagName; })()`);
  await sleep(50);
}
